...
class Settings(BaseSettings):
    ...
    # "full"   — monolith (local dev, loads everything)
    # "api"    — lightweight service, no ML models loaded
    # "ml"     — ML-heavy service, loads all models
    # "worker" — RQ worker + scheduler
    SERVICE_TYPE: str = "full"

    # Internal ML service URL (used by the "api" service to proxy ML requests)
    # Leave empty when running as "full" or "ml" — ML runs in-process
    ML_SERVICE_URL: str = ""

    # GCS model storage (production only)
    # Leave empty to use models from the local disk (dev/worker image)
    GCS_BUCKET_NAME: str = ""
    GCS_MODEL_PREFIX: str = "models/"

    # Override path to ML model files (set automatically by gcs_loader at runtime)
    ML_MODELS_DIR: str = ""

    # Internal service auth secret (shared between API and ML services)
